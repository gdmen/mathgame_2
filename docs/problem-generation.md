...
in `server/mathcore/enums.go` (`ProblemType` iota block, `problemTypeNames`);
...
(`generationFunnel.String`, `api/stamping.go`; the `lexer`/`unknown_rules` stages
...
1. Constant in `server/mathcore/enums.go` + `problemTypeNames` entry — feature-named, not subject-named.
...
- `server/mathcore/enums.go` — `ProblemType` bits, `problemTypeNames`, `ALL_PROBLEM_TYPES`, `WEIGHTED_TOPIC_MASK`
