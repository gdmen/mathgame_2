...
| `compare_generators` | `-cells`, `-samples`, `-seed` | informational heuristic_2.0 review (#283): reads the pool from a snapshot, puts fresh heuristic_2.0 output beside stored heuristic_1.0/llm rows in the same (bitmap, difficulty) cell, and reports LLM-offload + coverage gaps. Writes nothing; needs MySQL creds in `conf.json`. NOT a merge gate (the CI B-gate is). |
