...
// (migrations 18-27 applied) are consistent: events/gamestates/user_has_video/playlist_video
...
func main() {
	beforeConfig := flag.String("before-config", "", "path to config JSON for DB before migrations 18+")
	afterConfig := flag.String("after-config", "", "path to config JSON for DB after migrations 18+")
	flag.Parse()
...
// canonicalKey matches migration 25: youtu.be/ID, or v=ID, else url.
