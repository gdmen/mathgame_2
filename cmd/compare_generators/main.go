...
// (#283). It is NOT a merge gate (the CI B-gate in
// server/generator/heuristic2_test.go is) — it is the human-review tool: it
// reads the real problem pool from a prod snapshot, finds the symbolic cells
// users actually get served and who fills them today, then puts fresh
// heuristic_2.0 output side by side with the stored heuristic_1.0 / llm_* rows
// in the same (bitmap, difficulty) cell so the pedagogy and naturalness can be
// eyeballed. It also reports how much of the symbolic LLM volume heuristic_2.0
// could absorb (the LLM-offload story) and which cells it cannot reach
// in-window (the documented coarse-concept / floor-ceiling gaps).
//
// Writes nothing. Mirrors cmd/diagnose_generation's shape; needs a config with
