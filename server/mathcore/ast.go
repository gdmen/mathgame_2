...
// ComputeProblemDifficulty). There is no Parse or AST-native Eval yet: the
// token-cursor evaluator remains authoritative, and this AST is a construction
// scaffold whose only consumer today is the builder. Promoting it to the
// canonical representation (Parse+Eval, AST-native detection) is deferred to the
// EXPONENTS work, where a new precedence level is the forcing function.
//
// Render is NOT used for admission storage — Admission.Expr keeps the original
// notation. It exists for builder construction only.
