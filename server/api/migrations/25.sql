...
-- Step 3b: Backfill user_has_video from videos.user_id so Step 4 can remap loser->winner for user-owned videos. (Skip if videos has no user_id.)
SET @sql = IF((SELECT COUNT(*) FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_SCHEMA=DATABASE() AND TABLE_NAME='videos' AND COLUMN_NAME='user_id') > 0, 'INSERT IGNORE INTO user_has_video (user_id, video_id) SELECT user_id, id FROM videos WHERE user_id > 0', 'SELECT 1');
...
-- Step 7b: Ensure every video with a valid user_id has a row in user_has_video. (Skip if videos has no user_id.)
SET @sql = IF((SELECT COUNT(*) FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_SCHEMA=DATABASE() AND TABLE_NAME='videos' AND COLUMN_NAME='user_id') > 0, 'INSERT IGNORE INTO user_has_video (user_id, video_id) SELECT user_id, id FROM videos WHERE user_id > 0', 'SELECT 1');
PREPARE stmt FROM @sql;
EXECUTE stmt;
DEALLOCATE PREPARE stmt;

-- Step 8: Remove user_id from videos. (Skip if column does not exist.)
SET @sql = (SELECT IF(
  (SELECT COUNT(*) FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME = 'videos' AND COLUMN_NAME = 'user_id') > 0,
