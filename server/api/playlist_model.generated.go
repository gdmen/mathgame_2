...
const (
	...
	createPlaylistSQL = `INSERT INTO playlists (you_tube_id, title, thumbnailurl, etag) VALUES (?, ?, ?, ?);`

	getPlaylistSQL = `SELECT * FROM playlists WHERE id=?;`

	getPlaylistKeySQL = `SELECT id FROM playlists WHERE you_tube_id=? AND title=? AND thumbnailurl=? AND etag=?;`
...
func (m *PlaylistManager) Create(model *Playlist) (int, string, error) {
	...
	result, err := m.DB.Exec(createPlaylistSQL, model.YouTubeId, model.Title, model.ThumbnailURL, model.Etag)
	if err != nil {
		...
		err = m.DB.QueryRow(getPlaylistKeySQL, model.YouTubeId, model.Title, model.ThumbnailURL, model.Etag).Scan(&model.Id)
		if err != nil {
			msg := "Couldn't add playlist to database"
			return http.StatusInternalServerError, msg, err
		}
