...
func (a *Api) processEvent(logPrefix string, c *gin.Context, event *Event, writeCtx bool, user *User, gamestate *Gamestate, settings *Settings) error {
	...
	if event.EventType == LOGGED_IN {
		...
		if parseErr != nil || val < 3 || val > ceiling {
			msg := fmt.Sprintf("Invalid target_difficulty: %s (must be 3-%.1f for the current problem types)", event.Value, ceiling)
		...
		// settings UI's validateBitmap (#225 PR3); an API client bypassing
