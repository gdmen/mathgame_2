...
// anonymized users row, with the one free-text field scrubbed in
// anonymizeAndPurgeUser) so aggregate solve-time data survives for difficulty
...
// customDeleteAccount deletes the authenticated user's account: it confirms
// intent with the adult PIN, purges per-user state and anonymizes the users row
// in a single transaction, then best-effort removes the Auth0 identity so the
// same login can't re-claim the (now anonymized) record.
...
// the retained events stay grouped by a stable key that no live account can
...
func (a *Api) anonymizeAndPurgeUser(userID uint32) (err error) {
	...
	// A bad-problem report is the one event a person types prose into, so the
	// explanation is scrubbed while the row stays: the report is still evidence
	// about the problem, it just stops being anything about a person. The
	// JSON_VALID guard skips the pre-JSON rows whose value is a bare id.
	if _, err = tx.Exec(
		`UPDATE events SET value=JSON_SET(value, '$.explanation', '')
		 WHERE user_id=? AND event_type=? AND JSON_VALID(value)
		   AND JSON_EXTRACT(value, '$.explanation') IS NOT NULL`,
		userID, BAD_PROBLEM_USER,
	); err != nil {
		return err
	}
