...
// seededPerUserTables are the tables seedUserData puts a row in, plus the two
// (settings, gamestates) the create-user flow owns. Every one is asserted
// non-empty before a delete and empty after, so a seed that silently inserts
// nothing can't turn a purge assertion into a tautology.
var seededPerUserTables = []string{
	"settings",
	"gamestates",
	"review_queue",
	"recently_shown_problems",
	"user_has_video",
	"user_playlist",
	"statistics_cache_meta",
	"statistics_totals",
	"statistics_monthly",
}

// seedUserData gives the deletion something to purge in every per-user table.
// user_has_video and user_playlist are FK-bound to videos/playlists, so those
// parents are created first — seeding them with hardcoded ids would insert
...
func seedUserData(t *testing.T, api *Api, userID uint32) {
	...
	stmts := []struct {
		...
		{"INSERT INTO statistics_totals (user_id, total_problems_solved) VALUES (?, 1)", []interface{}{userID}},
		{"INSERT INTO statistics_monthly (user_id, month, total_problems_solved) VALUES (?, '2026-01', 1)", []interface{}{userID}},
		{"INSERT IGNORE INTO gamestates (user_id, problem_id, video_id, solved, target) VALUES (?, 1, ?, 0, 3)", []interface{}{userID, videoIDs[0]}},
		{"INSERT INTO events (user_id, event_type, value) VALUES (?, ?, '42')", []interface{}{userID, ANSWERED_PROBLEM}},
		{"INSERT INTO events (user_id, event_type, value) VALUES (?, ?, '7')", []interface{}{userID, SELECTED_PROBLEM}},
		{"INSERT INTO events (user_id, event_type, value) VALUES (?, ?, ?)", []interface{}{userID, BAD_PROBLEM_USER,
			`{"problem_id":1,"explanation":"my kid Alex can't read this one"}`}},
	}
	for _, s := range stmts {
	...
	for _, tbl := range seededPerUserTables {
		if n := countRows(t, api, "SELECT COUNT(*) FROM "+tbl+" WHERE user_id=?", userID); n == 0 {
			t.Fatalf("seedUserData: %s is empty, so a purge assertion on it would be vacuous", tbl)
		}
	}
}
...
func TestDeleteAccount_CorrectPin_PurgesAndAnonymizes(t *testing.T) {
	...
	// events rows are retained (aggregate value) and still keyed to the user id.
	if n := countRows(t, api, "SELECT COUNT(*) FROM events WHERE user_id=?", user.Id); n != eventsBefore {
		t.Errorf("events should be retained, count=%d (want %d)", n, eventsBefore)
	}
	// ...but the one free-text field a person typed into is scrubbed.
	var explanation string
	if err := api.DB.QueryRow(
		"SELECT JSON_UNQUOTE(JSON_EXTRACT(value, '$.explanation')) FROM events WHERE user_id=? AND event_type=?",
		user.Id, BAD_PROBLEM_USER).Scan(&explanation); err != nil {
		t.Fatalf("read retained report event: %v", err)
	}
	if explanation != "" {
		t.Errorf("report explanation should be scrubbed, got %q", explanation)
	}
