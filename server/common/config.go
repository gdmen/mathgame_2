...
type Config struct {
	...
	// Auth0 identity (see server/api/delete_account.go). Excluded from
	// Validate() via optionalConfigFields so existing configs stay valid.
...
// optionalConfigFields are json field names skipped by Validate() — they may
// be empty without failing startup.
