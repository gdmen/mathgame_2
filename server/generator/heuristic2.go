...
//     splits choose the dividend as an exact multiple (a = v*b), integers stay
//     integers unless a fraction/decimal/percent concept is active, and values
//     stay non-negative unless NEGATIVES is enabled.
...
// aimed at target difficulty. Returns (expression, answer, error). The heuristic
// emits only symbolic problems (no WORD), so there is no symbolic_expression.
...
func chooseAnswer(ctx buildCtx) *big.Rat {
	...
	switch {
	case ctx.fractions:
		// A fraction answer with a rich-ish denominator the splits can divide.
		d := pickDenom(ctx)
...
func withinMag(v *big.Rat, ctx buildCtx) bool {
	...
	den := v.Denom()
	cap := big.NewInt(int64(ctx.maxOperand))
